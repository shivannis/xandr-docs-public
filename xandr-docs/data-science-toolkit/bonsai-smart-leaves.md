...
| `OBJECT[ID].recency` | float, integer | The number of minutes since the user has seen an ad. This can be determined for all ads under an advertiser, line item, or campaign, or for an individual creative. If no recency data is found, no bid will be made.<br>`OBJECT` is `advertiser`, `line_item`, or `campaign`, or `creative`, and `ID` is the object ID. Use the [Advertiser Service](../digital-platform-api/advertiser-service.md), [Line Item Service](../digital-platform-api/line-item-service.md), or [Campaign Service](../digital-platform-api/campaign-service.md) to retrieve IDs. |
