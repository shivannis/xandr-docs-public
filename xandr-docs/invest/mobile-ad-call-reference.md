...
| `dnt` | boolean | No | **Note: This field is deprecated, and may be removed in the future.** Use the `LimitAdTrackingEnabled` flag instead.<br><br>If `true`, the user has set the system-level preference to not receive behaviorally targeted ads. Allowed values: `true`, `false`, `1`, or `0`. |
