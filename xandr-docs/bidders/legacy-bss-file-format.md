...
| `TIMESTAMP` | int | The time at which you would like your segment to go into effect. A [UNIX timestamp](https://en.wikipedia.org/wiki/Unix_time), down to the second. If a timestamp is submitted to greater specificity (down to the millisecond, for example) then the user will not be properly written to the segment upon upload. | The current time. | `1278250469` |
