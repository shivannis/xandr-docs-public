...
| `geo_dma_id` | int | Yes | The ID of the user's designated market area location as defined by the [Designated Market Area Service](./designated-market-area-service.md).<br><br>**Note**: **Why am I seeing a DMA ID of `1`**?<br><br>Our reporting derives DMA from the city logged for the auction. However, our geo provider is sometimes unable to determine a city from the IP address associated with the impression, even when DMA **is** determined. Therefore, there are cases where a campaign targeting a specific DMA has impressions in reporting showing a DMA of `1`. |
