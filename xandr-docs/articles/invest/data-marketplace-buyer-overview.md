...
<b>Note:</b> The cost of any segments for
...
<td class="entry" headers="ID-000086c8__entry__3"><a
href="mailto:DataGuru@acxiom.com" class="xref"
...
<b>Important:</b> For information regarding
...
<b>Important:</b> Data rates are not inclusive
...
<b>Note:</b> If you ever choose to deactivate
...
1.  When selecting targeting options on the line
    item, if you select the
    Edit button next to
    Segment under
    Targeting, the
...
href="xandr-api/data-usage-report.md"
...
**Member Analytics** report contains the sum of
all third-party data costs. For details on these data costs, see
<a href="data-usage-report.md" class="xref">Data Usage Report</a>. In
...
<a href="data-usage-report.md" class="xref">Data Usage Report</a>. For
...
line item under the insertion order.
...
parameters within an ad request. If a line item
is targeting 3rd party segments and there are no relevant segments then
no bid is made. A used segment is a segment that is used to bid in the
auction. The difference between a relevant segment and a used segment is
that not all relevant segments are required to be used for a
line item to bid in the auction. Note that many
other factors besides segment targeting influence the decision on
...
2.  Used segments are determined based upon the segments which caused
    Microsoft Invest to submit a bid. The list of
...
In the simplest scenario, the line item is
targeting just a single segment. The presence of the user's unique
identifier (a parameter in the auction) determines whether the
demographic segment is relevant. For line item
targeting one segment if a segment is relevant, then the segment will be
used to bid in the auction.
...
If multiple segments are targeted with a line
item, the boolean AND/ OR logic that the buyer has selected will
determine whether the ad request matches the required targeting
criteria.
...
- If the ad request is relevant in ALL targeted segments, a bid will be
  ...
  ![Advanced And-a](media/advanced_and_a.png)


- If the ad request is not relevant to one or many of the targeted
  segments, then no bid will be made

  ![Advanced And-b](media/advanced_and_b.png)
  ![Advanced And-c](media/advanced_and_c.png)
  

  

  <b>Note:</b> Since the ad request is
...
- If the ad request is relevant to ANY segment, a bid will be made using
  ...
  ![Advanced Or-a](media/advanced_or_a.png)
  ![Advanced Or-b](media/advanced_or_b.png)
...
- If the ad request is relevant to more than one segment (overlaps),
 ...
 ![Advanced Or-c](media/advanced_or_c.png)

- If the ad request is NOT relevant to any targeted segment, then no bid
  will be made.

 ![Advanced Or-d](media/advanced_or_d.png)
...
the line item bids in the auction or not.

**AND Targeting using Segment Groups**

![Advanced And](media/advanced_and.png)
...
![Advanced Or](media/advanced_or.png)
...
- If a parameter in the ad request
...
**Do I have to change anything about my line
item to make this work?**

If your in-flight line item is targeting a
segment supported by the data marketplace, then there is nothing on the
line item that you have to change. Once the data
provider is activated for clearing, Xandr will
immediately start clearing the cost of data on your behalf.
...
- **Used Segment** - A segment that is used by
  ...
  line item wins the auction, then all used
