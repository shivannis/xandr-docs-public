...
> [!NOTE]
> **Xandr Seller Tags:** When using [Xandr's seller tag (AST)](../seller-tag/seller-tag.md), omit the `kw_` prefix. AST has the keyword object in the body of the request, so the prefix is not needed.
