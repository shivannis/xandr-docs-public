...
| Field | Type | Description |
|:---|:---|:---|
| `name` | string | The name of the macro to be replaced in the any of the creative's urls (media, pixel, click, etc) or content. The name may contain any characters except the carat (^), and must be formatted as `${MACRO_NAME}`<br>within the creative's urls or content.<br><br>**Note:** Custom macros are replaced AFTER system macros have been replaced.<br>See [Creative Service](creative-service.md) for more details on custom macros.<br><br>**Required:** No |
