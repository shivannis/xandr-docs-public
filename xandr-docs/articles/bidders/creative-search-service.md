...
| POST        | [https://api..com/creative-search?sort=FIELD_NAME.ASC_OR_DESC](https://api..com/creative-search?sort=FIELD_NAME.ASC_OR_DESC) (creative_search JSON) | Search for creatives and sort results. **Note**: The fields used to search for creatives are not exactly the same as the fields returned. This is because the returned objects are drawn from the [Creative Service](creative-service) and formatted accordingly. This service will only search active creatives. |
