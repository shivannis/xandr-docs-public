...
Tip: Most of the below options can be
performed during pixel export in Monetize. For
...
option in Monetize when you export a pixel. Or
...
targeted in your line item or campaign. Please
note that whether or not a user is subject to segment actions is
independent of whether or not the conversion was attributed.

This pixel adds the user to two segments, and removes them from one:

``` pre
 <img src="https://ib.adnxs.com/px?id=8371&seg=12192,12304&remove=23145&t=2" width="1" height="1" /> 
```



Note: There are two ways to add a user
...
``` pre
<img src="https://ib.adnxs.com/px?id=1&order_id=[ORDER_ID]&t=2" width="1" height="1" /> 
```
...
line item or campaign, we cannot optimize to a
moving target, so we will optimize to the static value entered at the
line item level.

``` pre
<img src="https://ib.adnxs.com/px?id=1&value=[REVENUE]&t=2" width="1" height="1" /> 
```



Warning: The revenue value passed in
...
When you create a conversion pixel in Monetize,
you can input the URL of your pixel and mark it as image or JavaScript.
Please note that following restrictions:
...
- There are no character limits to piggybacked pixels in
  ...
  ``` pre
  <img src="https://ib.adnxs.com/px?id=1&redir=[REDIRECT_URL]&t=2" width="1" height="1" /> 
  ```
...
``` pre
<img src="https://ib.adnxs.com/px?id=1&other=[EXTERNAL_DATA]&t=2" width="1" height="1" /> 
```
