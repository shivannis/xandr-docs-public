...
<a href="reporting-guide.html" class="xref">Reporting Guide</a> will be
...
Tip: **Valid Carrier Codes for running
Debug Auctions**
...
Important: Note that device ID fields
are case sensitive.
...
Note: Several of these parameters need
...
Note: <strong>This field is deprecated,
...
Note: The `id` parameter in these
...
Important: If no response format is
...
Important: In order to receive a JSON
...
Note: No JSON response is returned if
no ad is returned.
...
Important: In order to receive an HTML
...
Note: Native ads are not available from
the `/ssmob` endpoint.
