...
| `schain` | string | Required for requests on resold inventory | `ptv`, `ssptv` | Advertising systems should support a parameter in their ad tags or VAST URLs to accept a string-serialized SupplyChain. It is recommended that this parameter be `schain`. For more information about `schain` string parameters and their corresponding JSON objects, see: SupplyChain for Non-OpenRTB Requests<br><br>`1.0,1!exchange1.com,1234,1,bid-request-1,publisher,publisher.com!exchange2.com,abcd,1,bid-request2,intermediary,intermediary.com` |
