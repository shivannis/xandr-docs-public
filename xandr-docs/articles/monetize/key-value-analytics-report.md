...
body of the (`ut/v3`) request, so the prefix is not needed. The same
holds true for other POST-based request body ad calls from Prebid
(`ut/v3/prebid`, `openrtb2/prebid`, `prebid/lfv`) and OpenRTB
(`openrtb2`).
...
Important: Lifetime Analytics Data
...
a given advertiser or publisher. This is due to the fact that Analytics
...
earned by this Network on the impression.</td>
</tr>
<tr class="even row">
...
creative)</td>
<td class="entry" headers="ID-00001f71__entry__62">The number of
page-level roadblocks that served the master creative. Note: This metric
